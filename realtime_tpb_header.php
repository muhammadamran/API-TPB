...
// CHECK DATABASE LOCAL referensi_pengusaha
// ROLE SYNC: BASE ON ID
// SHOW VALUE TABLE referensi_pengusaha
$dataTPB_L = $db->query("SELECT * FROM referensi_pengusaha", 0);
// LIST COUNT TABLE referensi_pengusaha
foreach ($dataTPB_L as $row_L) {
    if (@$row_L['ID']) {
        $ID = $row_L['ID'];
        // CHECK DATABASE DOMAIN referensi_pengusaha
        $dataTPB_D = $dbmain->query("SELECT * FROM referensi_pengusaha WHERE ID='$ID'");
        $row_D = mysqli_fetch_array($dataTPB_D);
        // IF DATA ID ALREADY INSERT ON DATABASE DOMAIN
        if ($row_D['ID'] == NULL) {
            // COUNT ROW TABLES referensi_pengusaha FROM DATABASE LOCAL
            $CountTPB_L_to_D = $db->query("SELECT COUNT(*) total_row FROM referensi_pengusaha WHERE ID='$ID'");
            $row_Count_L_to_D = $CountTPB_L_to_D->fetch_assoc();
            $pcs = $row_Count_L_to_D['total_row'];
            // ROW TABLES referensi_pengusaha FROM DATABASE LOCAL
            $dataTPB_L_to_D = $db->query("SELECT * FROM referensi_pengusaha WHERE ID='$ID'");
            while ($list_L_to_D = $dataTPB_L_to_D->fetch_assoc()) {
                $queryRP = $dbmain->query("INSERT INTO referensi_pengusaha
                                        (ID,ALAMAT,CONTACT_PERSON,EMAIL,FAX,ID_PENGENAL,JENISTPB,KODE_ID,KODE_KANTOR,NAMA,NOMOR_PENGENAL,NOMOR_SKEP,NPWP,STATUS_IMPORTIR,TANGGAL_SKEP,TELEPON)
                                        VALUES
                                        ('','$list_L_to_D[ALAMAT]','$list_L_to_D[CONTACT_PERSON]','$list_L_to_D[EMAIL]','$list_L_to_D[FAX]','$list_L_to_D[ID_PENGENAL]','$list_L_to_D[JENISTPB]','$list_L_to_D[KODE_ID]','$list_L_to_D[KODE_KANTOR]','$list_L_to_D[NAMA]','$list_L_to_D[NOMOR_PENGENAL]','$list_L_to_D[NOMOR_SKEP]','$list_L_to_D[NPWP]','$list_L_to_D[STATUS_IMPORTIR]','$list_L_to_D[TANGGAL_SKEP]','$list_L_to_D[TELEPON]')
                                        ");
            }
        } else {
            // IF ALREADY SYNC CHECK UPDATE DATA
            // COUNT ROW TABLES referensi_pengusaha FROM DATABASE LOCAL
            $UpdateCountTPB_L_to_D = $db->query("SELECT COUNT(*) total_row FROM referensi_pengusaha WHERE ID='$ID'");
            $Updaterow_Count_L_to_D = $UpdateCountTPB_L_to_D->fetch_assoc();
            $Updatepcs = $Updaterow_Count_L_to_D['total_row'];
            // ROW TABLES referensi_pengusaha FROM DATABASE LOCAL
            $UpdatedataTPB_L_to_D = $db->query("SELECT * FROM referensi_pengusaha WHERE ID='$ID'");
            while ($Updatelist_L_to_D = $UpdatedataTPB_L_to_D->fetch_assoc()) {
                $queryRP = $dbmain->query("UPDATE referensi_pengusaha SET
                                                                    ALAMAT='$Updatelist_L_to_D[ALAMAT]',
                                                                    CONTACT_PERSON='$Updatelist_L_to_D[CONTACT_PERSON]',
                                                                    EMAIL='$Updatelist_L_to_D[EMAIL]',
                                                                    FAX='$Updatelist_L_to_D[FAX]',
                                                                    ID_PENGENAL='$Updatelist_L_to_D[ID_PENGENAL]',
                                                                    JENISTPB='$Updatelist_L_to_D[JENISTPB]',
                                                                    KODE_ID='$Updatelist_L_to_D[KODE_ID]',
                                                                    KODE_KANTOR='$Updatelist_L_to_D[KODE_KANTOR]',
                                                                    NAMA='$Updatelist_L_to_D[NAMA]',
                                                                    NOMOR_PENGENAL='$Updatelist_L_to_D[NOMOR_PENGENAL]',
                                                                    NOMOR_SKEP='$Updatelist_L_to_D[NOMOR_SKEP]',
                                                                    NPWP='$Updatelist_L_to_D[NPWP]',
                                                                    STATUS_IMPORTIR='$Updatelist_L_to_D[STATUS_IMPORTIR]',
                                                                    TANGGAL_SKEP='$Updatelist_L_to_D[TANGGAL_SKEP]',
                                                                    TELEPON='$Updatelist_L_to_D[TELEPON]'
                                                                    WHERE ID='$Updatelist_L_to_D[ID]'");
            }
        }
    }
    ...
    <div class="wrapper ">
        <section class="login_content">
            <form action="logedin.php" method="POST">
                ...
                <div
                    style="display:grid;justify-content:center;align-items:center;margin-bottom: 25px;font-size: 18px;font-weight: 600;">
                    <div class="text-center">
