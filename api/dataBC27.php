...
$dataBC27 = $db->query("SELECT *,sts.KODE_STATUS,sts.URAIAN_STATUS 
                        FROM tpb_header AS hdr 
                        JOIN referensi_status AS sts ON hdr.KODE_STATUS=sts.KODE_STATUS 
                        WHERE hdr.KODE_DOKUMEN_PABEAN=27 GROUP BY hdr.NOMOR_AJU ORDER BY hdr.NOMOR_AJU", 0);
$cek = $dataBC27->num_rows;

if ($cek > 0) {
    $data = [];

    while ($result = $dataBC27->fetch_assoc()) {
        $data[] = [
