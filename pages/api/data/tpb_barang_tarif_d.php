...
if (isset($_POST['findLimit_D'])) {
    $data = $dbmain->query("SELECT * FROM tpb_barang_tarif ORDER BY ID_HEADER DESC, JENIS_TARIF ASC LIMIT " . $_POST['Limit_D'] . "", 0);
} else {
    $data = $dbmain->query("SELECT * FROM tpb_barang_tarif ORDER BY ID_HEADER DESC, JENIS_TARIF ASC LIMIT 100", 0);
}
