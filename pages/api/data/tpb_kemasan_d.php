...
if (isset($_POST['findLimit_D'])) {
    $data = $dbmain->query("SELECT * FROM tpb_kemasan ORDER BY ID DESC LIMIT " . $_POST['Limit_D'] . "", 0);
} else {
    $data = $dbmain->query("SELECT * FROM tpb_kemasan ORDER BY ID DESC LIMIT 100", 0);
}
...
?>
    <tr>
        ...
        <td style="text-align: center;"><?= $row['KODE_JENIS_KEMASAN']; ?></td>
        <td style="text-align: center;"><?= $row['MERK_KEMASAN']; ?></td>
        <td style="text-align: center;"><?= $row['NIP_GATE_IN']; ?></td>
        <td style="text-align: center;"><?= $row['NIP_GATE_OUT']; ?></td>
        <td style="text-align: center;"><?= $row['NO_POLISI']; ?></td>
        <td style="text-align: center;"><?= $row['NOMOR_SEGEL']; ?></td>
        <td style="text-align: center;"><?= $row['SERI_KEMASAN']; ?></td>
