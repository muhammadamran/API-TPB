...
if (isset($_POST['findLimit_L'])) {
    $data = $db->query("SELECT * FROM tpb_bahan_baku_tarif ORDER BY ID_HEADER DESC, JENIS_TARIF ASC LIMIT " . $_POST['Limit_L'] . "", 0);
} else {
    $data = $db->query("SELECT * FROM tpb_bahan_baku_tarif ORDER BY ID_HEADER DESC, JENIS_TARIF ASC LIMIT 100", 0);
}
