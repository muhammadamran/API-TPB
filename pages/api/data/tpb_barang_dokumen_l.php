...
if (isset($_POST['findLimit_L'])) {
    $data = $db->query("SELECT * FROM tpb_barang_dokumen ORDER BY ID DESC LIMIT " . $_POST['Limit_L'] . "", 0);
} else {
    $data = $db->query("SELECT * FROM tpb_barang_dokumen ORDER BY ID DESC LIMIT 100", 0);
}
